...
exeを作成する場合は PyInstaller を使います。

```powershell
py -3.12 -m pip install pyinstaller
py -3.12 -m PyInstaller --noconfirm --clean --onedir --windowed --name LyricsToEXO app.py
```

生成物は `dist/LyricsToEXO/` に作成されます。配布するときは、`LyricsToEXO.exe` 単体ではなく `dist/LyricsToEXO/` フォルダごとzip化してください。
